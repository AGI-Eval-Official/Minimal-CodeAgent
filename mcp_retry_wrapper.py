...
class RobustMcpSessionManager(_BaseSessionManager):
    ...
    async def create_session_with_retry(self, headers: Optional[Dict[str, str]] = None):
        ...
        for attempt in range(self.retry_config.max_retries + 1):
            try:
                ...
                async with asyncio.timeout(self.retry_config.timeout):
                    # Call base class's actual creation method to avoid recursion
        ...
        # All retries failed
        logger.error(f"MCP connection failed after {self.retry_config.max_retries} retries")
        raise last_exception

    async def create_session(self, headers: Optional[Dict[str, str]] = None):
        """Override base class create_session, default to retry version."""
...
class RobustMcpToolset(_BaseMCPToolset):
    ...
    def __init__(self, *args, retry_config: Optional[RetryConfig] = None, **kwargs):
        # First initialize normally with base class to get _connection_params / _errlog
        super().__init__(*args, **kwargs)
        self.retry_config = retry_config or RetryConfig()
        self._fallback_tools = []
        self._last_error: Optional[str] = None
        # Use same connection_params/errlog to build robust session manager, replace base class's
        try:
            ...
            logger.warning(f"Failed to build RobustMcpSessionManager, falling back to original manager: {e}")
    
    async def get_tools(self, readonly_context=None):
        """Override original method to ensure no exceptions are thrown even if MCP fails, returns degradable results.
        This way upper FastAPI won't return 500, client can get JSON normally.
        """
        try:
            ...
            # Return degraded tools or empty list to avoid throwing exceptions that cause 500
            if self._fallback_tools:
                logger.info("Using degraded tool list")
                return self._fallback_tools
            logger.warning("Returning empty tool list")
            return []
    
    async def get_tools_with_retry(self, ctx):
        """Tool acquisition with retry and degradation"""
        try:
            ...
            # Return degraded tools or empty list
            if self._fallback_tools:
                logger.info("Using degraded tool list")
                return self._fallback_tools
            else:
    ...
    def set_fallback_tools(self, tools: List[Any]):
        """Set degraded tool list"""
        self._fallback_tools = tools


def apply_mcp_monkey_patches(retry_config: Optional[RetryConfig] = None):
    """Replace ADK's MCP classes with robust versions to avoid exceptions causing /run to return 500.
    - Replace McpSessionManager with RobustMcpSessionManager, create_session with retry
    - Replace McpToolset with RobustMcpToolset, get_tools catches exceptions and returns degraded results
    """
    try:
        ...
        # If lowercase class name exists, override
        if original_toolset_lower is not None:
            mcp_toolset_module.McpToolset = PatchedMcpToolset  # type: ignore[attr-defined]
        # If uppercase class name exists, override
        if original_toolset_upper is not None:
            mcp_toolset_module.MCPToolset = PatchedMcpToolset  # type: ignore[attr-defined]

        logger.info("✅ MCP monkey patch applied: McpSessionManager and McpToolset using robust versions")
        return {
            "patched": True,
...
async def robust_mcp_connection(
    ...
    try:
        ...
        # Can implement degradation logic here
        yield None
        ...
        if session:
            try:
                ...
                logger.warning(f"Error occurred while closing MCP session: {e}")
    ...
    async def setup_robust_agent_tools(agent, mcp_configs: List[Dict[str, Any]]):
        ...
        for config in mcp_configs:
            try:
                ...
                # Set degraded tools (if needed)
                if config.get("enable_fallback"):
                    fallback_tools = []  # Can add backup tools here
...
async def example_robust_mcp_usage():
    ...
    async with robust_mcp_connection(mcp_config, retry_config) as mcp_session:
        if mcp_session:
            ...
            logger.warning("MCP connection failed, using degraded mode")
            # Degradation logic
