...
class LiteLlmWithSleep(LiteLlm):
    ...
    max_total_tokens: int = Field(default=50_000_000, description="Max total tokens for truncate")
    ...
    def _get_session_id(self, llm_request: LlmRequest) -> str:
        ...
        # Get session_id from request's custom_metadata, generate one if not available
    ...
    def _create_exit_loop_response(self, session_id: str = None, reason=None) -> LlmResponse:
        ...
        # Mark that early stop has been triggered
        if session_id:
    ...
    def force_reset_early_stop(self):
        ...
        import logging
        logger = logging.getLogger(__name__)
        logger.info("Early stop status reset for all sessions")
...
"""
    ...
    async def generate_content_async(
        ...
        # Add token warning information to request (if exceeding warning threshold)
        ...
        try:
            ...
            # If exception occurs, also trigger early stop
            
            logger.error(f"Exception occurred while generating content: {e}")
            self._set_session_early_stop(session_id, True, reason=f"Exception occurred while generating content: {e}")
            yield self._create_exit_loop_response(session_id, reason=str(e))
            return
